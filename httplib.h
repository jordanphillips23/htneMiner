...
public:
  ...
  explicit Client2(const char *scheme_host_port,
    ...
    if (std::regex_match(scheme_host_port, m, re)) {
      ...
      auto port = !port_str.empty() ? atoi(port_str.c_str())
                                    : (scheme == "https" ? 443 : 80);
  ...
  if (std::regex_match(s, m, re_first_range)) {
    ...
    split(&s[pos], &s[pos + len], ',', [&](const char *b, const char *e) {
      ...
      if (std::regex_match(b, e, cm, re_another_range)) {
        ssize_t first = -1;
        if (!cm.str(1).empty()) {
          first = static_cast<ssize_t>(atoll(cm.str(1).c_str()));
        }

        ssize_t last = -1;
        if (!cm.str(2).empty()) {
          last = static_cast<ssize_t>(atoll(cm.str(2).c_str()));
        }
...
inline std::string make_content_range_header_field(size_t offset, size_t length,
  ...
  field += "-";
  field += to_string(offset + length - 1);
  field += "/";
  field += to_string(content_length);
  return field;
}
...
inline std::pair<std::string, std::string> make_range_header(Ranges ranges) {
  ...
  for (auto r : ranges) {
    if (i != 0) { field += ", "; }
    if (r.first != -1) { field += to_string(r.first); }
    field += '-';
    if (r.second != -1) { field += to_string(r.second); }
    i++;
  }
...
Server::process_request(Stream &strm, bool last_connection,
  ...
  req.set_header("REMOTE_PORT", to_string(req.remote_port));
...
inline bool Client::read_response_line(Stream &strm, Response &res) {
  ...
  if (std::regex_match(line_reader.ptr(), m, re)) {
    ...
    res.status = atoi(std::string(m[2]).c_str());
  }
...
inline bool Client::write_request(Stream &strm, const Request &req,
  ...
  if (req.body.empty()) {
    if (req.content_provider) {
      auto length = to_string(req.content_length);
